...
describe('Decimal', () => {
  ...
  describe('multiplication and division', () => {
    ...
    // Regression: divide() used to round on a nudged extra digit, which carried
    // into the next place whenever the quotient ended in 9, and truncated the
    // dividend when the target scale sat below the operand scale.
    it('rounds on the true remainder rather than a carried digit', () => {
      // 679 / 2000 = 0.3395 exactly.
      expect(Decimal.parse('679').divide(Decimal.parse('2000'), 2, 'DOWN').toString()).toBe('0.33');
      expect(Decimal.parse('679').divide(Decimal.parse('2000'), 2, 'HALF_UP').toString()).toBe('0.34');
      // 60 / 59 = 1.016949... — the fourth decimal must not round up.
      expect(Decimal.parse('60').divide(Decimal.parse('59'), 4, 'HALF_UP').toString()).toBe('1.0169');
      expect(Decimal.parse('60').divide(Decimal.parse('59'), 4, 'HALF_EVEN').toString()).toBe('1.0169');
    });

    it('keeps the digits that decide rounding when scaling down', () => {
      // GBp -> GBP: the trailing 1 is what pushes this up to 33.45.
      expect(Decimal.parse('3344.5001').divide(Decimal.parse('100'), 2, 'HALF_UP').toString()).toBe(
        '33.45',
      );
      expect(Decimal.parse('3344.4999').divide(Decimal.parse('100'), 2, 'HALF_UP').toString()).toBe(
        '33.44',
      );
      // Exactly half: HALF_UP goes away from zero, HALF_EVEN to the even digit.
      expect(Decimal.parse('3344.5').divide(Decimal.parse('100'), 2, 'HALF_UP').toString()).toBe(
        '33.45',
      );
      expect(Decimal.parse('3344.5').divide(Decimal.parse('100'), 2, 'HALF_EVEN').toString()).toBe(
        '33.44',
      );
    });

    it('rounds negative quotients by the same rule as positive ones', () => {
      expect(Decimal.parse('-679').divide(Decimal.parse('2000'), 2, 'DOWN').toString()).toBe('-0.33');
      expect(Decimal.parse('-679').divide(Decimal.parse('2000'), 2, 'FLOOR').toString()).toBe('-0.34');
      expect(Decimal.parse('679').divide(Decimal.parse('-2000'), 2, 'CEILING').toString()).toBe('-0.33');
      expect(Decimal.parse('-679').divide(Decimal.parse('-2000'), 2, 'DOWN').toString()).toBe('0.33');
    });
  });
