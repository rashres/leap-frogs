...
export class Decimal {
  ...
  divide(other: Decimal, targetScale: number, mode: RoundingMode): Decimal {
    ...
    // Line the operands up so the quotient lands exactly at targetScale, then round
    // once against the true remainder. When the shift is negative the divisor is
    // scaled up rather than the dividend truncated, so no digit that decides the
    // rounding is discarded.
    const shift = targetScale + other.scale - this.scale;
    let numerator = shift >= 0 ? this.unscaled * pow10(shift) : this.unscaled;
    let denominator = shift >= 0 ? other.unscaled : other.unscaled * pow10(-shift);
    // Keep the divisor positive so the remainder's sign tracks the quotient's.
    if (denominator < 0n) {
      numerator = -numerator;
    ...
    return new Decimal(round(quotient, remainder, denominator, mode), targetScale);
  }
