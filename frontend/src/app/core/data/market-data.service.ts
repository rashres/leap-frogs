...
export class MarketDataService {
  ...
  private failures = 0;
  /** Identifies the current poll chain, so superseded ones stop rescheduling. */
  ...
  series(instrumentId: string, range: ChartRange): readonly PricePoint[] {
    ...
    // A fresh array is handed back rather than the cached one mutated in place:
    // signal inputs compare by reference, so an in-place edit would leave every
    // chart frozen at its first render while the price beside it ticked on.
    const tail = cached[cached.length - 1];
    if (tail && !tail.price.equals(quote.rate)) {
      const next = [...cached.slice(0, -1), { at: new Date(), price: quote.rate }];
      this.seriesCache.set(key, next);
      return next;
    }
    return cached;
  }
  ...
  private restartPolling(): void {
    ...
    this.timer = null;
    this.failures = 0;
    // Retire any poll already in flight: comparing the source alone is not
    // enough, because switching away and back again matches it once more and
    // would leave two chains running against a throttled endpoint.
    this.pollGeneration += 1;
    void this.poll(this.pollGeneration);
  }
  ...
  private async poll(generation: number): Promise<void> {
    ...
    try {
      ...
      if (provider.id !== 'fixture' && generation === this.pollGeneration) {
        this.failures += 1;
      ...
      if (generation === this.pollGeneration) {
        this.timer = setTimeout(() => void this.poll(generation), this.nextDelay(provider.pollMs));
      }
    }
  }
