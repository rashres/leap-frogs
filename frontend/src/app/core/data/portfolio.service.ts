...
export class PortfolioService {
  ...
  applyFill(input: {
    ...
    const settle = (available: Money): Money =>
      side === 'BUY' ? available.minus(consideration) : available.plus(consideration);

    const held = this.cashBalances();
    const nextCash = held.some((balance) => balance.currency === currency)
      ? held.map((balance) =>
          balance.currency === currency ? { ...balance, available: settle(balance.available) } : balance,
        )
      : // No balance in this currency yet — open one rather than drop the money.
        // A sale in an unheld currency would otherwise settle into nowhere.
        [
          ...held,
          {
            accountId: this.accountId,
            currency,
            available: settle(Money.zero(currency)),
          },
        ];
