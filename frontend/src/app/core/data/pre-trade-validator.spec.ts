...
describe('PreTradeValidator', () => {
  ...
  // Regression: an accepted order reserves nothing while it waits to fill, so
  // validation has to net off what is already in flight or the same balance
  // passes twice and cash goes negative once both settle.
  it('will not let two orders in flight spend the same cash', () => {
    const btc = market.requireInstrument('crypto-btc');
    const quantity = orders.maxBuyQuantity(btc, priceOf(btc.id));

    const first = orders.place({
      instrument: btc,
      side: 'BUY',
      quantity,
      indicativeQuote: market.latestQuote(btc.id),
    });
    expect(first.state).toBe('ACCEPTED');

    const second = orders.place({
      instrument: btc,
      side: 'BUY',
      quantity,
      indicativeQuote: market.latestQuote(btc.id),
    ...
    expect(second.rejectionReason?.code).toBe('INSUFFICIENT_CASH');
  });

  it('will not let two orders in flight sell the same holding', () => {
    // Crypto, so the 24/7 session keeps this independent of the wall clock.
    const instrument = market.requireInstrument('crypto-eth');
    const quantity = portfolio.heldQuantity(instrument.id);

    const first = orders.place({
      instrument,
      side: 'SELL',
      quantity,
      indicativeQuote: market.latestQuote(instrument.id),
    });
    expect(first.state).toBe('ACCEPTED');
    ...
    expect(second.state).toBe('REJECTED');
    expect(second.rejectionReason?.code).toBe('INSUFFICIENT_HOLDINGS');
  });
