...
export class OrderService {
  ...
  /**
   * Cash committed by working BUY orders in one currency, at their indicative
   * price. An accepted order has not settled yet, so this is what stops the same
   * balance being spent twice inside the execution delay.
   */
  private committedCash(currency: CurrencyCode): Money {
    let total = Money.zero(currency);
    for (const order of this.workingOrders()) {
      if (order.side !== 'BUY' || order.currency !== currency) continue;
      total = total.plus(this.considerationFor(order.indicativeQuote.price, order.quantity));
    }
    return total;
  }

  /** Quantity committed by working SELL orders on one instrument. */
  private committedQuantity(instrumentId: string): Decimal {
    let total: Decimal | null = null;
    for (const order of this.workingOrders()) {
      if (order.side !== 'SELL' || order.instrumentId !== instrumentId) continue;
      total = total ? total.plus(order.quantity) : order.quantity;
    }
    return total ?? Decimal.zero(0);
  }
  ...
  preview(request: Omit<PlaceOrderRequest, 'indicativeQuote'> & { price: Money }): PreTradeOutcome {
    const consideration = this.considerationFor(request.price, request.quantity);
    return this.validator.validate({
      instrument: request.instrument,
      side: request.side,
      quantity: request.quantity,
      consideration,
      committedCash: this.committedCash(consideration.currency),
      committedQuantity: this.committedQuantity(request.instrument.id),
    });
  }
  ...
  place(request: PlaceOrderRequest): Order {
    ...
    const now = new Date();
    // Read commitments before this order joins the list, or it would count itself.
    const consideration = this.considerationFor(indicativeQuote.price, quantity);
    const committedCash = this.committedCash(consideration.currency);
    const committedQuantity = this.committedQuantity(instrument.id);
  ...
  maxBuyQuantity(instrument: Instrument, price: Money): Decimal {
    ...
    // Net off cash already committed by working buys, so the figure offered here
    // is one that validation will actually accept.
    const cash = this.portfolio.cashFor(instrument.currency).minus(this.committedCash(instrument.currency));
    if (!cash.amount.isPositive()) return Decimal.zero(policy.quantityScale);
