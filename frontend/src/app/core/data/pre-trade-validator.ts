...
export interface PreTradeRequest {
  ...
  readonly at?: Date;
  /**
   * Cash and quantity already spoken for by orders that are accepted but not yet
   * settled. Settled balances alone are not enough: an order reserves nothing
   * while it is in flight, so without these two a client could pass validation
   * twice over the same funds and overdraw once both fill.
   */
  readonly committedCash?: Money | null;
  readonly committedQuantity?: Decimal | null;
}
...
export class PreTradeValidator {
  ...
  validate(request: PreTradeRequest): PreTradeOutcome {
    ...
    if (side === 'BUY') {
      const settled = this.portfolio.cashFor(consideration.currency);
      const committed = request.committedCash;
      const available =
        committed && committed.currency === settled.currency ? settled.minus(committed) : settled;
      ...
      const settled = this.portfolio.heldQuantity(instrument.id);
      const committed = request.committedQuantity;
      const held = committed ? settled.minus(committed) : settled;
